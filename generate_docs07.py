...
class CParser:
    ...
    def _finalize_enum(self, match, line):
        ...
        # Strip C comments from enum body before parsing
        ...
        for p in parts:
            p = p.strip()
            if not p: continue
            vm = re.match(r'(\w[\w_]*)\s*(=\s*[^,]+?)?\s*(?:\/\*\*\s*<\s*(.*?)\s*\*\/)?\s*$', p, re.DOTALL)
            if vm:
                vn = vm.group(1)
    ...
    def _finalize_struct(self, match, line):
        ...
        body = match.group(1) if match.lastindex >= 1 else ""
        # Strip C comments from body before parsing
        body = re.sub(r'/\*.*?\*/', '', body, flags=re.DOTALL)
        from collections import namedtuple
        ...
        for part in body.split(';'):
            ...
            # Function pointer: int (*name)(params) - parse and continue
            fp_m = re.match(r'(.*?)\s*\(\s*\*\s*(\w[\w_]*)\s*\)\(.*', p)
            if fp_m:
                e.struct_fields.append(Field(fp_m.group(1).strip() + '(*)', fp_m.group(2).strip(), doc))
                continue
            # Skip non-pointer member functions
            if '(' in p and ')' in p: continue
            words = p.split()
            if len(words) >= 2:
                fname_candidate = words[-1].rstrip(';]')
                ftype = ' '.join(words[:-1])
                ...
                fname = fname.lstrip('*')
                if fname == '':
                    fname = fname_candidate  # fallback
    ...
    # ------------------------------------------------------------------
    # feed_line dispatchers — each handles one syntactic pattern
    # ------------------------------------------------------------------

    def _try_block_comment(self, stripped):
        """Skip /***... copyright banners (not Doxygen /**)."""
    ...
    def _try_end_skip_block(self, stripped):
        """Check if we're inside a skipped block comment and it just ended."""
    ...
    def _try_doc_continuation(self, stripped):
        """Continuation line inside a multi-line /** ... */ comment."""
    ...
    def _try_accum_continue(self, line, stripped):
        """Continue accumulating a multi-line block (typedef/struct/enum)."""
    ...
    def _try_start_accum(self, stripped, line):
        ...
        for kw in [r'typedef\s+(?:enum|struct)\s*(?:\w+\s*)?\{',
                   r'enum\s+\w+\s*\{',
                   r'struct\s+\w+\s*\{']:
    ...
    def _try_typedef(self, stripped, line):
        """Single-line typedef enum/struct or simple typedef."""
        m = re.match(r'typedef\s+enum\s*\{(.*?)\}\s*(\w[\w_]*)\s*;', stripped, re.DOTALL)
        if m: self._finalize_enum(m, line); return True
        m = re.match(r'typedef\s+struct\s*\{(.*?)\}\s*(\w[\w_]*)\s*;', stripped, re.DOTALL)
        if m: self._finalize_struct(m, line); return True
    ...
    def _try_define(self, stripped, line):
        ...
        if m:
            ...
            e.func_return = val  # store value in func_return
            self._doc_accum = []
            return True
        return False

    def _try_const_var(self, stripped, line):
        """const/constexpr global variable declaration."""
        m = re.match(r'(?:const|constexpr|static\s+const|const\s+static)\s+(?:\w[\w\s\*]*)\s+(\w[\w_]*)\s*(?:=|{)', stripped)
    ...
    def _try_function(self, stripped, line):
        ...
        # Multi-line function accumulation: line has ( but no ) yet
        if getattr(self, '_accum_func', False):
            self._accum_func.append(line)
            if ');' in stripped or (stripped.endswith(')') and ')' in stripped):
                full = ' '.join(l.strip() for l in self._accum_func)
                self._accum_func = False
                e = self._parse_func(full)
                return bool(e)
            return True  # consumed by accumulation
        
        # Single-line function
        if (stripped and not stripped.startswith("#") and first not in func_skip
        ...
        # Start multi-line function accumulation: has ( but no )
        if (stripped and not stripped.startswith("#") and first not in func_skip
            and not stripped.startswith("//") and not stripped.startswith("/*")
    ...
    def _finalize_accum_block(self, line):
        """When a multi-line block closes, determine its type and finalize."""
        full = '\n'.join(self._accum_block[0])
        self._accum_block = False
 
        self._accum_func = False
        from collections import namedtuple
        # C-style typedef enum { ... } name; or typedef enum name { ... } alias;
        m = re.match(r'typedef\s+enum\s*(?:\w+\s*)?\{(.*)\}\s*(\w[\w_]*)\s*;', full, re.DOTALL)
        if m: self._finalize_enum(m, line); return
        # C-style typedef struct { ... } name; or typedef struct name { ... } alias;
        m = re.match(r'typedef\s+struct\s*(?:\w+\s*)?\{(.*)\}\s*(\w[\w_]*)\s*;', full, re.DOTALL)
        ...
        if m:
            ...
            for part in body_clean.split(';'):
                ...
                fp_m = re.match(r'(.*?)\s*\(\s*\*\s*(\w[\w_]*)\s*\)\(.*', p)
                if fp_m:
                    e.struct_fields.append(Field(fp_m.group(1).strip() + '(*)', fp_m.group(2).strip(), ''))
                    continue
                if '(' in p and ')' in p: continue
                words = p.split()
                if len(words) >= 2:
    ...
    def feed_line(self, line):
        ...
        stripped = line.strip()
        if self._try_block_comment(stripped):   return
        if self._try_end_skip_block(stripped):  return
        self._try_trailing_doc(stripped)
        if self._try_doc_comment(stripped):     return
        if self._try_doc_continuation(stripped): return
        if self._try_accum_continue(line, stripped): return
        if self._try_start_accum(stripped, line):    return
        if self._try_class(stripped, line):    return
        if self._try_typedef(stripped, line):  return
        if self._try_define(stripped, line):   return
        if self._try_const_var(stripped, line): return
        if self._try_function(stripped, line): return
        # Default: clear doc accumulator if we weren't building a doc
        if stripped and not stripped.startswith(("//","/*")) and not self._in_doc_block:
            self._doc_accum = []
...
'''
    # Files section: use dropdown select instead of inline tabs
    # Determine which option is selected
    is_globals_sub = active_section in ("func","type","enum","eval","defs","vars")
    sel_filelist = " selected" if active_section == "files" else ""
    sel_globals = " selected" if active_section == "globals" else ""
    sel_func = " selected" if active_section == "func" else ""
    sel_type = " selected" if active_section == "type" else ""
    sel_enum = " selected" if active_section == "enum" else ""
    sel_eval = " selected" if active_section == "eval" else ""
    sel_defs = " selected" if active_section == "defs" else ""
    sel_vars = " selected" if active_section == "vars" else ""
    return f'''  <div id="navrow2" class="tabs2" style="padding:4px 10px;">
    <select onchange="window.location=this.value;" style="font-size:13px;padding:2px 6px;">
      <option value="files.html"{sel_filelist}>文件列表</option>
      <option value="globals.html"{sel_globals}>全局定义 - 全部</option>
      <option value="globals_func.html"{sel_func}>全局定义 - 函数</option>
      <option value="globals_type.html"{sel_type}>全局定义 - 类型定义</option>
      <option value="globals_enum.html"{sel_enum}>全局定义 - 枚举</option>
      <option value="globals_eval.html"{sel_eval}>全局定义 - 枚举值</option>
      <option value="globals_defs.html"{sel_defs}>全局定义 - 宏定义</option>
      <option value="globals_vars.html"{sel_vars}>全局定义 - 变量</option>
    </select>
  </div>
'''
...
def nav_tabs(active_page="main"):
    def li(page, label):
        ...
        return f'<li{cls}><a href="{href}"><span>{label}</span></a></li>'
    # Build "文件" tab with dropdown submenu
    files_current = ' class="current"' if active_page == "files" else ''
    # Determine which sub-item to mark as selected
    sub_sel = {"files":"文件列表", "func":"函数", "type":"类型定义", "enum":"枚举",
               "eval":"枚举值", "defs":"宏定义", "vars":"变量", "globals":"全部"}.get(active_page, "")
    sub_selected = sub_sel  # just for display
    def sub_link(href, label):
        return f'<a href="{href}" style="display:block;padding:6px 16px;text-decoration:none;color:#283A5D;white-space:nowrap;">{label}</a>'
    # Dropdown via onmouseover/onmouseout (no CSS hover dependency)
    submenu_items = [
        ("files.html", "文件列表"),
    ...
    submenu_html = ""
    for i, (href, label) in enumerate(submenu_items):
        if i == 1:
            submenu_html += f'<div style="border-top:1px solid #C4CFE5;margin:4px 0;"></div>\n'
        submenu_html += f'    <a href="{href}" style="display:block;padding:4px 16px;text-decoration:none;color:#283A5D;white-space:nowrap;font-size:13px;">{label}</a>\n'
    
    files_tab = f'''<li{files_current} style="position:relative;"
    onmouseover="document.getElementById('files-dropdown').style.display='block';"
    onmouseout="document.getElementById('files-dropdown').style.display='none';">
  <a href="files.html"><span>文件</span></a>
  <div id="files-dropdown" style="display:none;position:absolute;top:36px;left:0;background:white;border:1px solid #C4CFE5;border-radius:4px;box-shadow:0 2px 8px rgba(0,0,0,0.1);z-index:200;min-width:180px;padding:4px 0;">
{submenu_html}  </div>
</li>'''
    ...
    classes_sub_html = ""
    for href, label in classes_submenu:
        if label.startswith("─"):
            classes_sub_html += f'    <div style="border-top:1px solid #C4CFE5;margin:4px 0;"></div>\n'
        else:
            classes_sub_html += f'    <a href="{href}" style="display:block;padding:4px 16px;text-decoration:none;color:#283A5D;white-space:nowrap;font-size:13px;">{label}</a>\n'
    
    classes_tab = f'''<li{classes_current} style="position:relative;"
    onmouseover="document.getElementById('classes-dropdown').style.display='block';"
    onmouseout="document.getElementById('classes-dropdown').style.display='none';">
  <a href="annotated.html"><span>结构体</span></a>
  <div id="classes-dropdown" style="display:none;position:absolute;top:36px;left:0;background:white;border:1px solid #C4CFE5;border-radius:4px;box-shadow:0 2px 8px rgba(0,0,0,0.1);z-index:200;min-width:160px;padding:4px 0;">
{classes_sub_html}  </div>
</li>'''
...
'''
    ...
    def render_file_page(self, fpath):
        ...
        summary_links = []
        kind_labels = {"struct":"结构体","enum":"枚举","function":"函数","typedef":"类型定义","define":"宏定义","variable":"变量"}
        for kind in ["struct","enum","function","typedef","define","variable"]:
            if kind in by_kind:
                anchor = {"struct":"struct-members","enum":"enum-members","function":"func-members","typedef":"typedef-members","define":"define-members","variable":"var-members"}[kind]
...
'''
    ...
    def _function_table(self, entities):
        ...
        for e in entities:
            brief = e.doc.brief or ""
            # Build signature with type-to-entity links
            parts = []
            for p in e.func_params:
                type_html = self._type_link(p.type.rstrip())
                parts.append(type_html + ' ' + p.name)
            sig = ', '.join(parts) if parts else ''
            rows += f'<tr class="memitem:f_{e.name}" id="r_f_{e.name}"><td class="memItemLeft">{E(e.func_return)}&#160;</td><td class="memItemRight"><a class="el" href="#r_f_{e.name}">{E(e.name)}</a>({sig})</td></tr>\n'
            if brief:
    ...
    def _simple_table(self, entities, kind):
        """Render defines or variables table."""
        ...
        for e in entities:
            p = kind[0]
            val = e.func_return if kind == 'define' and e.func_return else ''
            display = E(e.name)
            if val:
                # Truncate long multi-line values
                val_short = val.split('\n')[0][:60]
                display += ' = ' + E(val_short)
            rows += '<tr class="memitem:' + p + '_' + e.name + '"><td class="memItemLeft">' + label + '&#160;</td><td class="memItemRight">' + display + '</td></tr>' + NL
        return '<table class="memberdecls">' + NL + section_header(label, anchor) + rows + '</table>' + NL
    ...
    def _type_link(self, type_name):
        """Generate <a> link for a type reference to its entity page.
           Handles pointer and const types by extracting the base name."""
        # Strip trailing * and const to get base type name
    ...
    def _write_globals_page(self):
        ...
        # Category config: page_name, title, subtab_key, category_key
        cat_config = [
            ('globals.html', '全部', 'globals', 'all'),
            ('globals_func.html', '函数', 'func', 'function'),
            ('globals_vars.html', '变量', 'vars', 'vars'),
            ('globals_type.html', '类型定义', 'type', 'typedef'),
            ('globals_enum.html', '枚举', 'enum', 'enum'),
            ('globals_eval.html', '枚举值', 'eval', 'eval'),
            ('globals_defs.html', '宏定义', 'defs', 'defs'),
        ]
        
        # navrow3 tabs HTML
        def navrow3_tabs(active_key):
            tabs = ''
            for pname, label, _subtab, key in cat_config:
        ...
        for page_name, title, subtab, key in cat_config:
            items = cats[key]
            if key == 'eval':
            ...
            c += page_titlebar() + nav_tabs("files") + page_subtabs(subtab)
            c += navrow3_tabs(key)
...
class ChmGenerator:
    ...
    def __init__(self, files_dict, html_dir, chm_path):
        self.files = files_dict  # dict of {path: FileInfo}
    ...
    def generate_hhc(self):
        """Generate table of contents with directory hierarchy."""
        ...
        def add_li(name, local, indent=""):
            lines.append(f'{indent}<LI><OBJECT type="text/sitemap">')
            lines.append(f'{indent}  <param name="Name" value="{name}">')
            if local:
                lines.append(f'{indent}  <param name="Local" value="{local}">')
            lines.append(f'{indent}</OBJECT>')
        
        # Top level
        add_li("首页", "index.html")
        add_li("结构体", "annotated.html")
        add_li("全局定义", "globals.html")
        add_li("文件", "files.html")
        ...
        def emit_tree(node, indent=""):
            for key in sorted(node.keys()):
                if key == "__files__":
                    continue
                sub = node[key]
                has_children = bool(sub) or bool(sub.get("__files__"))
                if has_children:
                    lines.append(f'{indent}<LI><OBJECT type="text/sitemap">')
                    ...
                    for sk in sorted(sub.keys()):
                        if sk == "__files__":
                            continue
                        emit_tree({sk: sub[sk]}, indent + "  ")
                    # Emit files in this dir
                    for fp in sorted(sub.get("__files__", []), key=lambda x: x.name):
                        page = file_id(fp) + ".html"
                        lines.append(f'{indent}  <LI><OBJECT type="text/sitemap">')
                        lines.append(f'{indent}    <param name="Name" value="{fp.name}">')
                        lines.append(f'{indent}    <param name="Local" value="{page}">')
                        lines.append(f'{indent}  </OBJECT>')
                    lines.append(f'{indent}</UL>')
