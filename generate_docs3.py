    ...
    def _write_globals_page(self):
        ...
        for page_name, title, key in cat_config:
            ...
            if key == 'eval':
                ...
                # Macros/defines (we don't have define entities, so skip)
                c += '<p>(No macro definitions documented)</p>'
            elif key == 'vars':
                # Variables (we don't track variables separately)
                c += '<p>(No global variables documented)</p>'
            else:
                # Regular entities
                for letter in letters:
                    c += f'<h3 class="doxsection"><a id="index_{letter.lower()}" name="index_{letter.lower()}"></a>- {letter} -</h3><ul>'
